...
import { cadastrarChamado, listarChamados, deletarChamado, atualizarChamado, buscarChamado } from "../repository/chamadoRepository.js";
import { buscarTecnico } from "./tecnicoController.js";
...
export function fecharChamado(indice) {
    const chamados = buscarChamado(indice);
...
export function assumirChamado(indiceTecnico, indiceChamado){
    const tecnico = buscarTecnico(indiceTecnico)
    const chamado = buscarChamado(indiceChamado)

    if (!tecnico || !chamado){
        return false;
    }
    
    chamado.assumirChamado(tecnico);
    return true;
}